...
- **FR-018**: Truncated results MUST include safe continuation guidance. Deterministic continuation MUST use stable ordering/keyset semantics where possible; if no stable order exists, the system MUST identify partial coverage and require a narrower ordered query rather than implying absence or completeness.
